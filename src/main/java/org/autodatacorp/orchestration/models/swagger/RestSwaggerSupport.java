...
import org.apache.camel.swagger.*;
...
/**
 * Created by nurico on 2017-02-03.
 */
public class RestSwaggerSupport {
    ...
    private static void addOperationId(Swagger swagger){
        try {
            for (String pathKey : swagger.getPaths().keySet()){
                ...
                for (HttpMethod httpMethod : path.getOperationMap().keySet()){
                    ...
                    String operationId = httpMethod.name().toLowerCase();
                    String secondId = "";
                    if (operation.getTags() != null && operation.getTags().size() > 0){
                        secondId = operation.getTags().get(0);
                    } else if (operation.getSummary() != null){
                        secondId = operation.getSummary().replaceAll(" ","");
                    }
                    if (secondId.toLowerCase().startsWith(operationId)){
                        operationId = "";
                    }
                    operation.setOperationId(operationId + secondId);
                }
